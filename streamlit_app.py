...
def check_password():
    def password_entered():
        if st.session_state["password"] == "icm2026": # <-- Mude a senha aqui
...
with aba3:
    ...
    # BAIXAR EXCEL
    if st.button("Baixar Excel com tudo"):
        with pd.ExcelWriter("relatorio_icm.xlsx") as writer:
            st.session_state.escala_df.to_excel(writer, sheet_name="Escala", index=False)
            st.session_state.frequencia_df.to_excel(writer, sheet_name="Frequencia", index=False)
        st.success("Arquivo 'relatorio_icm.xlsx' pronto!")
...
with aba4:
    ...
    if not st.session_state.frequencia_df.empty:
        ...
        # Gráfico 1: Total de pessoas por mês
        ...
        ax2.plot(df['Data do Culto'], df['% Visitantes'], marker='o', color='#000000')
